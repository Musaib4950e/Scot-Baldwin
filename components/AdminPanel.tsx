...
const AdminPanel: React.FC<AdminPanelProps> = (props) => {
    ...
    const closeAllModals = () => { setIsEditUserModalOpen(false); setIsPasswordModalOpen(false); setIsEditGroupModalOpen(false); setIsVerificationModalOpen(false); setIsGrantModalOpen(false); setSelectedUser(null); setSelectedGroup(null); };
    ...
    return (
        <>
                             ...
                             <div className="bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        <thead className="border-b border-white/10 text-sm text-slate-400"><tr><th className="p-4">User</th><th className="p-4">Balance</th><th className="p-4 text-right">Actions</th></tr></thead>
                                        <tbody>
                                            {users.filter(u => !u.isAdmin).sort((a,b) => b.walletBalance - a.walletBalance).map(user => (
                                                <tr key={user.id} className="border-b border-white/10 hover:bg-white/5 text-sm">
                                                    ...
                                                    <td className="p-4 text-right">
                                                        <button onClick={() => openGrantModal(user)} className="px-4 py-2 text-xs bg-green-500/20 hover:bg-green-500/30 text-green-300 rounded-lg font-semibold transition-colors">Grant Funds</button>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
            ...
            <div className={`fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ${isEditUserModalOpen || isPasswordModalOpen || isEditGroupModalOpen || isVerificationModalOpen || isGrantModalOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className={`bg-slate-900/50 backdrop-blur-2xl rounded-2xl p-6 w-full max-w-lg border border-white/10 shadow-2xl flex flex-col transition-all duration-300 ${isEditUserModalOpen || isPasswordModalOpen || isEditGroupModalOpen || isVerificationModalOpen || isGrantModalOpen ? 'scale-100' : 'scale-95'}`}>
                    <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold text-white">{isEditUserModalOpen && `Edit ${selectedUser?.username}`}{isPasswordModalOpen && `Reset Password for ${selectedUser?.username}`}{isEditGroupModalOpen && `Edit ${selectedGroup?.name}`}{isVerificationModalOpen && `Manage Verification for ${selectedUser?.username}`}{isGrantModalOpen && `Grant Funds to ${selectedUser?.username}`}</h2><button onClick={closeAllModals} className="p-1 text-slate-400 hover:text-white"><XMarkIcon /></button></div>
                    ...
                    {isGrantModalOpen && selectedUser && (<div className="space-y-4"><div><label className="block text-sm font-medium text-slate-300 mb-1">Amount to Grant</label><input type="number" value={grantAmount} onChange={e => setGrantAmount(e.target.value)} placeholder="Enter amount" className="w-full bg-white/5 border border-white/10 rounded-lg py-2 px-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" /></div><div className="mt-6 flex justify-end gap-3"><button onClick={closeAllModals} className="px-5 py-2.5 bg-white/10 hover:bg-white/20 rounded-lg transition-colors font-semibold">Cancel</button><button onClick={handleAdminGrantFunds} disabled={!grantAmount.trim() || isSubmitting} className="px-5 py-2.5 bg-green-600 hover:bg-green-500 rounded-lg transition-colors font-semibold disabled:bg-slate-600 disabled:cursor-not-allowed">{isSubmitting ? 'Granting...' : 'Confirm Grant'}</button></div></div>)}
